...
import { cachedDetection, SERVER_PRO_TIMEOUT_MS, withTimeout } from "@/lib/demoCache";

// Judges one post's TEXT (caption, comments) — from the demo sandbox or a real account's
// public feed. The payload has no field that could carry an image.
export async function POST(req: Request) {
  ...
  let result = null;
  try {
    result = await withTimeout(detectPost(post, context), SERVER_PRO_TIMEOUT_MS);
  } catch {}
