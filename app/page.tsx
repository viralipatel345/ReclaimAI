...
const statusTone = (st: string) =>
  st === "Removed" ? "bg-[#E7F6EC] text-[#166534]" :
  st === "Overdue" ? "bg-[#0E1116] text-white" : "bg-[#FDECEA] text-[#B3130F]";
  ...
  return (
    <div className="relative mx-auto w-[300px] rounded-[48px] bg-[#0E1116] p-[10px] shadow-[0_40px_80px_-30px_rgba(14,17,22,0.55)]">
      <div className="relative h-[600px] overflow-hidden rounded-[40px] bg-[#F7F7F8]">
          ...
          <AnimatePresence initial={false}>
            {SCRIPT.slice(0, n).map((b, i) => (
              <motion.div key={i} layout initial={{ opacity: 0, y: 10, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.35, ease: EASE }}
                ...
                {b.text}
                {b.card === "platform" && <div className="mt-2 flex items-center gap-2 rounded-xl bg-[#FDECEA] px-2.5 py-1.5 text-[11px] font-semibold text-[#B3130F]"><Check size={12} /> {REDDIT.platform} · covered · {REDDIT.channel}</div>}
                {b.card === "request" && (
                ...
                {b.card === "clock" && <div className="mt-2 flex items-center justify-between rounded-xl bg-[#0E1116] px-2.5 py-2 text-white"><span className="text-[10.5px] font-semibold">{REDDIT.platform}</span><Clock deadline={sentAt.current + 48 * 3600e3} className="text-[13px] font-semibold" /></div>}
              </motion.div>
            ))}
            {typing && (
        ...
        <a href={APP_URL} className="absolute inset-x-3 bottom-3 flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-[12px] text-[#9CA3AF] shadow-[0_2px_10px_rgba(0,0,0,0.06)] hover:text-[#4B5563]">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ delay: i * 0.08, duration: 0.6, ease: EASE }} whileHover={{ y: -4 }}
      ...
      <div className="flex items-center justify-between">
        ...
        {"sandbox" in a && (a as { sandbox?: boolean }).sandbox && <span className="rounded-full bg-[#0E1116] px-2.5 py-1 text-[11px] font-semibold text-white">Demo sandbox</span>}
      </div>
...
export default function LandingPage() {
  return (
    <div className="bg-white font-[var(--font-plus-jakarta),sans-serif] text-[#0E1116] antialiased">
      <Header />
      <main>
        <Hero />
        <Process />
