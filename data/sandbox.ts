...
/**
 * The sandbox account, written around the case's own name and one of its reported links, so
 * the same scenario works for any case (the demo's fictional "Jordan Ellis" or a consenting
 * teammate's name in real mode). Post ids stay stable.
 */
export function sandboxAccountFor(fullName: string, knownUrl: string): SandboxAccount {
  const [firstRaw = "", ...rest] = fullName.trim().split(/\s+/);
  const lastRaw = rest[rest.length - 1] ?? "";
  const first = firstRaw.toLowerCase();
  const last = lastRaw.toLowerCase();
  const handle = last ? `@${first}.${last}` : `@${first}`;
  const initials = `${firstRaw[0] ?? ""}.${lastRaw[0] ?? ""}.`.toUpperCase();
  const link = knownUrl.replace(/^https?:\/\//, "");
  return {
    platformId: "instagram",
    handle: "example_account",
    displayName: "archive",
    bio: `new stuff every week · full archive → ${link}`,
    followers: 1284,
    following: 37,
    posts: [
      post("Cq7Lx2aPq1", "photo", "2026-09-24T21:10:00.000Z", `remember her? full set in bio ${handle}`, [
        { user: "vx_91", text: `no way this is ${first} ${last}`.trim() },
      ...
      post("Cq3Jb9eQs5", "reel", "2026-09-20T22:15:00.000Z", "new drop, link in bio", [{ user: "k_lo", text: `mirror: ${knownUrl}` }]),
      post("Cq2Hd4fVt8", "photo", "2026-09-19T12:00:00.000Z", "ramen night #foodie"),
      post("Cq1Gw7gXr3", "carousel", "2026-09-18T20:40:00.000Z", `${initials} part 2`, [{ user: "r.vale", text: "part 1 was better" }]),
      post("Cq0Fs2hZn6", "photo", "2026-09-17T16:20:00.000Z", "throwback to 2021", [{ user: "lincoln_alum", text: `wait is that ${first} from lincoln high??` }]),
      post("Cp9Eq5iLk9", "photo", "2026-09-16T10:05:00.000Z", "monday mood #memes", [{ user: "dee", text: "literally me" }]),
    ],
  };
}

/** The demo case's sandbox (fictional Jordan Ellis). */
export const SANDBOX_ACCOUNT: SandboxAccount = sandboxAccountFor("Jordan Ellis", DEMO_URLS.imgvault);
